...
public class Alien2 extends Charakter {
	...
	private static int FREQ=10; //sta³a zwi¹zana z czêstoœci¹ strzelania swtorka, im wiêksza tym czêœciej 
	...
	private CopyOnWriteArrayList<AlienShot2> shots= new CopyOnWriteArrayList<AlienShot2>(); //strza³y wygenerowane przez stworka
	
	public Alien2(int x, int y, int speed, int health, int freq, BufferedImage apperence,Game current) {
		super(x,y,speed);
	...
	protected void move() {//ruch stworka
		if(x_position<=0 && move_speed<0) { //lewa granica okna
			game.reverseSpeed2(); //odwraca kierunek ruchu wszystkich stworków w grze 
		}
	...
	public void render(Graphics g) {//wyœwietlanie stworka
		...
		Iterator<AlienShot2> iter = shots.iterator(); 
		while(iter.hasNext()) 
		iter.next().render(g);
	}
	...
	public boolean colision(int x, int y) { // czy nast¹pi³a kolizja stworka i strza³u gracza
		if(x_position>= (x-width/2) && x_position<=(x+ width/4) && y_position>=(y-height/2) && y_position<= (y+height/2)){
			if(this.health_points<=0) {
				game.getAliens2().remove(this); //usuñ stworka z gry
				game.addScore(3); //zwiêksz punkty
				game.addShots2(getShots());
	...
	private void shot() { //strza³ storka
		...
		if(rand<=FREQ) {
			...
			shots.add(new AlienShot2((x_position+width/4),(y_position+height/4),15,game.getImage().getAlienShot2(),game,this));
		}
	}
	
	public CopyOnWriteArrayList<AlienShot2> getShots(){ //dostêp do strza³ów
		return shots;
	}
