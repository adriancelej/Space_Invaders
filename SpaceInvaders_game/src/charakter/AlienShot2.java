...
public class AlienShot2 extends Charakter{
...
private Alien2 owner; //stworek do którego nale¿y

	
public AlienShot2(int x, int y, int speed, BufferedImage shot, Game game, Alien2 owner){
		super(x,y,speed);
		this.shot=shot;
		this.game=game;
...
private void checkColision() { //sprawd¿ kolizjê z graczem
	if(isRunning()) {
		if(game.getPlayer().colision(x_position, y_position)) { //czy kolizja nast¹pi³a
			...
			game.getAliensShot2().remove(this);
			game.getPlayer().decreaseHealth(2); //zmniejsz pkt ¿ycia gracza
			stop(); // zakoñcz w¹tek
			}
		}
}
